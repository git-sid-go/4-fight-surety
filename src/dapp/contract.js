...
export default class Contract {
  ...
  isOperational(callback) {
    let self = this;
    self.flightSuretyApp.methods.isOperational().call(
      {
        from: self.owner,
      },
      callback
  ...
  isAirline(airlineAccount, callback) {
    let self = this;
    self.flightSuretyApp.methods.isAirline(airlineAccount).call(
      {
        from: self.owner,
      },
      callback
  ...
  pay(passengerAccount, airlineAccount, airlineName, timestamp, callback) {
    let self = this;
    self.flightSuretyApp.methods.withdrawCredits().send(
      {
        from: passengerAccount,
      },
      callback
