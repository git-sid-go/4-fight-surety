...
contract("Flight Surety Tests", async (accounts) => {
  ...
  it(`(multiparty) has first airline registered automatically`, async function () {
    // Get operating status
    let status = await config.flightSuretyData.isAirline.call(
      config.firstAirline
    );
  ...
  it("(airline) can register only 4 airlines using registerAirline() without the need of consensus", async () => {
    ...
    let resultIsAirline3 = await config.flightSuretyData.isAirline.call(
      thirdAirline
    );
    let resultIsAirline4 = await config.flightSuretyData.isAirline.call(
      fourthAirline
    );
    let resultIsAirline5 = await config.flightSuretyData.isAirline.call(
      fifthAirline
    );
    // ASSERT
    assert.equal(
      resultIsAirline3,
      true,
      "First airline should be able to register a third airline if it has provided funding"
    );
    assert.equal(
      resultIsAirline4,
      true,
      "First airline should be able to register a fourth airline if it has provided funding"
    );
    assert.equal(
      resultIsAirline5,
      false,
      "First airline should not be able to register a fifth airline without consensus"
    );
  ...
  it("(airline) can register another airline with at least 50% of consensus", async () => {
    ...
    assert.equal(
      result,
      true,
      "Fifth airline should be registered by consensus"
    );
  ...
  it("(airline) cannot register another airline with less than 50% of consensus", async () => {
    ...
    assert.equal(
      result,
      false,
      "Sixth airline should not be registered without consensus"
    );
    assert.equal(await config.flightSuretyData.getAirlinesCount(), 5);
  });
  ...
  it("(passenger) can buy insurance for a flight paying up to 1 ether", async () => {
    ...
    // ASSERT
    let result = await config.flightSuretyData.getAmountPaidByInsuree.call(
      passengerAccount,
      config.firstAirline,
      airlineName,
      timestamp,
  ...
  it("(passengers) receive credit for the insurance bought and only once", async () => {
    ...
    await expectThrow(
      config.flightSuretyData.creditInsurees(
        insuranceReturnPercentage,
        config.firstAirline,
        airlineName,
        timestamp,
    ...
    // ASSERT
    // Passenger1
    let resultPassengerAmountPaid1 =
      await config.flightSuretyApp.getAmountPaidByInsuree.call(
        config.firstAirline,
        airlineName,
        timestamp,
        {
    ...
    let resultPassengerAmountCredit1 =
      await config.flightSuretyData.getInsureePayoutCredits.call(
        passengerAccount1,
        {
          from: passengerAccount1,
        }
    ...
    assert.equal(
      resultPassengerAmountCredit1,
      (passengerAmountPaid1 * insuranceReturnPercentage) / 100
    );
    // Passenger2
    let resultPassengerAmountPaid2 =
      await config.flightSuretyApp.getAmountPaidByInsuree.call(
        config.firstAirline,
        airlineName,
        timestamp,
    ...
    let resultPassengerAmountCredit2 =
      await config.flightSuretyData.getInsureePayoutCredits.call(
        passengerAccount2,
        {
          from: passengerAccount2,
    ...
    assert.equal(
      resultPassengerAmountCredit2,
      (passengerAmountPaid2 * insuranceReturnPercentage) / 100
    );
    // Airline fund
    assert.equal(
      BigNumber(fundsAfter).toNumber(),
      BigNumber(
        fundsBefore
          .add(resultPassengerAmountPaid1)
          .add(resultPassengerAmountPaid2)
          .sub(resultPassengerAmountCredit1)
          .sub(resultPassengerAmountCredit2)
      ).toNumber()
    );
  });
  ...
  it("(passenger) can withdraw credits successfully", async () => {
    ...
    let insuranceReturnPercentage = 150;
    let insuranceCredits =
      (passengerAmountPaid * insuranceReturnPercentage) / 100;
